...
public class BaseballActivity extends AppCompatActivity implements BaseballListFragment.BaseballListListener {


    int ghettoBackTracker = 0;
    //Gets called by the BaseballListFragment
    @Override
    public void updateWebView(int position) {
       // BaseballWebFragment webFragment = (BaseballWebFragment) getFragmentManager().findFragmentById(R.id.webFragment);
       // webFragment.setWebView(position);


        BaseballWebFragment webFragment = new BaseballWebFragment();
        webFragment.setWebView(position);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*
        BaseballListFragment listFragment = new BaseballListFragment();
        FragmentTransaction transaction = getFragmentManager().beginTransaction();
        //Replace whatever is in current container
        transaction.replace(R.id.container, listFragment);
        //Add to backstack
        transaction.addToBackStack(null);
        //Add and Commit
        //transaction.add(R.id.container, listFragment).commit();
        //Commit
        transaction.commit();
        */
    }
